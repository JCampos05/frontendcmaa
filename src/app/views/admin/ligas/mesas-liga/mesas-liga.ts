...
export class MesasLigaComponent implements OnInit {
  ...
  // --- Helpers de presentación (app-select / app-badge) ---
  // Nota: no alteran la lógica existente, solo adaptan los datos ya
  // calculados por los métodos de arriba al formato que esperan los
  // componentes de la librería de UI.
  // Precalculados (NUNCA getters que devuelvan arrays nuevos en el template —
  // Angular los reevalúa en cada ciclo de detección de cambios, causando NG0103).
  ligaSelectOptions: SelectOption<InfoLiga>[] = [];
  grupoSelectOptions: SelectOption<GrupoLiga>[] = [];
  rondaSelectOptions: SelectOption<number>[] = [];

  private actualizarOpcionesSelect(): void {
    this.ligaSelectOptions = this.ligasActivas.map(l => ({ value: l, label: l.nombre }));
    this.grupoSelectOptions = this.grupos.map(g => ({ value: g, label: `${g.nombre} (${g.rondas || 5} rondas)` }));
    this.rondaSelectOptions = this.rondasDisponibles.map(r => {
      let sufijo = '';
      if (r.estado === 'finalizada') sufijo = ' - Finalizada';
      else if (r.estado === 'en_curso') sufijo = ' - En curso';
      return { value: r.numeroRonda, label: `Ronda ${r.numeroRonda}${sufijo}` };
    });
  }

  onLigaSelectChange(liga: InfoLiga): void {
    this.ligaActual = liga;
    this.onLigaChange();
  }

  onGrupoSelectChange(grupo: GrupoLiga): void {
    this.grupoSeleccionado = grupo;
    this.onGrupoChange();
  }

  onRondaSelectChange(numeroRonda: number): void {
