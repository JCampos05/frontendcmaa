...
@Component({
  ...
  standalone: true,
  imports: [
    CommonModule, FormsModule, ToastNoti, PageHeaderComponent, StateMessageComponent, EmptyStateComponent,
    ButtonComponent, IconComponent, SelectComponent, StatCardGridComponent, DataTableComponent
  ],
...
export class HistorialJugadorComponent implements OnInit {
  ...
  readonly columnasPartidas: DataTableColumn[] = [
    { key: 'numeroRonda', label: 'Ronda', icon: 'flag' },
    { key: 'numeroMesa', label: 'Mesa', icon: 'grid-four' },
    { key: 'color', label: 'Color' },
    { key: 'rival', label: 'Rival', icon: 'user' },
    { key: 'resultado', label: 'Resultado' },
    { key: 'puntosObtenidos', label: 'Pts', align: 'center' },
    { key: 'puntosAcumulados', label: 'Acumulado', icon: 'chart-line', align: 'center' }
  ];
  ...
  categoriaOptions: SelectOption<number | null>[] = [];
  statCardsJugador: StatCardInput[] = [];

  private actualizarTorneoOptions(): void {
  ...
  private actualizarCategoriaOptions(): void {
    this.categoriaOptions = [
      { value: null, label: 'Todas las categorías' },
      ...this.categorias.map(c => ({ value: c.idTorneoCat!, label: c.categoria?.nombre || 'Categoría' }))
    ];
  }

  private actualizarStatCardsJugador(): void {
    if (!this.estadisticaJugador) {
      this.statCardsJugador = [];
      return;
    }
    const e = this.estadisticaJugador;
    this.statCardsJugador = [
      { icon: 'star', variant: 'info', label: 'Rating', value: e.jugador?.rating || 'S/R' },
      { icon: 'trophy', variant: 'brown', label: 'Puntos', value: e.puntos },
      { icon: 'grid-four', variant: 'teal', label: 'Partidas', value: e.partidas_jugadas },
      { icon: 'check-circle', variant: 'success', label: 'Victorias', value: e.victorias },
      { icon: 'handshake', variant: 'warning', label: 'Empates', value: e.empates },
      { icon: 'x-circle', variant: 'navy', label: 'Derrotas', value: e.derrotas }
    ];
  }
  ...
  onTorneoChange(value: number | null): void {
    this.torneoSeleccionado = value;
    this.categorias = [];
    ...
    this.actualizarStatCardsJugador();
