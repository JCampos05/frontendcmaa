...
@Component({
  selector: 'app-modal-resultado-partida-liga',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './resultado-partida-liga.html',
  styleUrls: ['./resultado-partida-liga.css']
})
export class ModalResultadoPartidaLigaComponent implements OnInit, OnDestroy, OnChanges {
  @Input() visible = false;
  @Input() mesa: MesaLiga | null = null;
  ...
  intentarBloquearMesa(): void {
    if (!this.mesa?.idMesaLiga) return;

    if (this.modoEdicion && this.mesa.partida) {
      this.cargarDatosExistentes();
    }

    const mesaDto: Partial<MesaLiga> = {
      usuarioEditando: this.usuarioActual
    };

    this.mesaLigaService.update(this.mesa.idMesaLiga, mesaDto).subscribe({
      next: (mesaActualizada) => {
        if (mesaActualizada.usuarioEditando === this.usuarioActual) {
          this.mesaBloqueada = false;
          this.mensajeValidacion = '';
          this.tipoMensaje = '';

          const timestamp = mesaActualizada.timestampEdicion;
          ...
          this.mostrarMensaje(
            `Esta mesa está siendo editada por ${mesaActualizada.usuarioEditando || 'otro usuario'}`,
            'error',
            0
          );
        }
      ...
      error: (err) => {
        ...
        if (err.status === 400 && err.error?.code === 'MESA_FINALIZADA') {
          this.mesaBloqueada = true;
  ...
  iniciarVerificacionConcurrencia(): void {
    ...
    this.intervaloVerificacion = setInterval(() => {
      if (this.mesa?.idMesaLiga && this.visible && !this.mesaBloqueada) {
  ...
  verificarEstadoMesa(): void {
    ...
    this.mesaLigaService.getById(this.mesa.idMesaLiga).subscribe({
      next: (mesaActualizada) => {
        if (mesaActualizada) {
  ...
  guardarIlegales(): void {
    if (!this.mesa?.idMesaLiga || this.mesaBloqueada) return;
  ...
  actualizarPartidaExistente(): void {
    if (!this.mesa?.partida?.idPartidaLiga) return;
    ...
    this.partidaLigaService.update(this.mesa.partida.idPartidaLiga, updatePartidaDto).subscribe({
      next: () => {
  ...
  guardarResultado(): void {
    if (!this.formularioValido() || !this.mesa?.idMesaLiga || this.mesaBloqueada) return;

    this.guardando = true;
    this.mensajeValidacion = '';
    this.tipoMensaje = '';

    if (this.modoEdicion && this.mesa.partida?.idPartidaLiga) {
    ...
    const partidaDto = {
      idMesaLiga: this.mesa.idMesaLiga,
      resultado: this.resultado,
      tipo_finalizacion: this.tipoFinalizacion!,
      descripcion_finalizacion: this.descripcionFinalizacion || undefined,
      duracion_minutos: this.duracionMinutos || undefined,
    ...
    this.partidaLigaService.create(partidaDto).subscribe({
      next: () => {
        this.actualizarMesa();
      },
      error: (err) => {
